...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-white rounded-xl shadow p-6 text-center">
        <p class="text-4xl font-bold text-green-600 mb-1">{{ $totalPemesanan }}</p>
        <p class="text-gray-500 text-sm">Total Pemesanan</p>
    </div>
    <div class="bg-white rounded-xl shadow p-6 text-center">
        <p class="text-4xl font-bold text-green-600 mb-1">{{ $totalLapangan }}</p>
        <p class="text-gray-500 text-sm">Total Lapangan</p>
    </div>
    <div class="bg-white rounded-xl shadow p-6 text-center">
        <p class="text-4xl font-bold text-green-600 mb-1">{{ $totalUser }}</p>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
    ...
    <a href="/jadwal"
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm">Lihat & edit jadwal lapangan</p>
        </div>
    </a>
    ...
    <a href="/admin/konfirmasi"
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm">Approve/reject bukti pembayaran user</p>
        </div>
    </a>
    ...
    <table class="w-full text-sm">
        ...
        <tbody>
            ...
            <tr class="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td class="px-4 py-3">{{ optional(optional($p->jadwal)->lapangan)->nama_lapangan ?? '-' }}</td>
                <td class="px-4 py-3">{{ optional($p->jadwal)->tanggal ?? '-' }}</td>
                ...
                <td class="px-4 py-3">
                    ...
                    @elseif($p->status_pemesanan == 'menunggu_konfirmasi')
                        <span class="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full">🔄 Menunggu Konfirmasi</span>
                    @elseif($p->status_pemesanan == 'dibayar')
                        <span class="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full">✓ Lunas</span>
